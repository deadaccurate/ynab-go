...
func (sts *ScheduledTransactionsService) List(budgetId string) ([]ScheduledTransactionDetail, error) {
	...
	if err := service(*sts).do("GET", "budgets/"+budgetId+"/scheduled_transactions", nil, nil, &response); err != nil {
		return nil, err
	}
...
func (sts *ScheduledTransactionsService) Get(budgetId string, scheduledTransactionId string) (ScheduledTransactionDetail, error) {
	...
	if err := service(*sts).do("GET", "budgets/"+budgetId+"/scheduled_transactions/"+scheduledTransactionId, nil, nil, &response); err != nil {
