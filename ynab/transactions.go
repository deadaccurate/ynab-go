...
func (ts *TransactionsService) List(budgetId string) ([]TransactionDetail, error) {
	...
	if err := service(*ts).do("GET", "budgets/"+budgetId+"/transactions", nil, nil, &response); err != nil {
		return nil, err
	}
...
func (ts *TransactionsService) Get(budgetId string, transactionId string) (TransactionDetail, error) {
	...
	if err := service(*ts).do("GET", "budgets/"+budgetId+"/transactions/"+transactionId, nil, nil, &response); err != nil {
...
func (ts *TransactionsService) Create(budgetId string, transaction *SaveTransaction) (TransactionDetail, error) {
	var response TransactionResponse
	if err := service(*ts).do("POST", "budgets/"+budgetId+"/transactions", nil, SaveTransactionWrapper{transaction}, &response); err != nil {
		return TransactionDetail{}, err
	}
	return response.Data.Transaction, nil
}
...
func (ts *TransactionsService) CreateBulk(budgetId string, transactions []SaveTransaction) ([]TransactionDetail, error) {
	var response TransactionsResponse
	if err := service(*ts).do("POST", "budgets/"+budgetId+"/transactions/bulk", nil, BulkTransactions{transactions}, &response); err != nil {
		return nil, err
	}
	return response.Data.Transactions, nil
}
...
func (ts *TransactionsService) Edit(budgetId string, transactionId string, transaction *SaveTransaction) (TransactionDetail, error) {
	var response TransactionResponse
	if err := service(*ts).do("PUT", "budgets/"+budgetId+"/transactions/"+transactionId, nil, SaveTransactionWrapper{transaction}, &response); err != nil {
		return TransactionDetail{}, err
	}
	return response.Data.Transaction, nil
}
...
func (ts *TransactionsService) GetByAccount(budgetId string, accountId string) ([]TransactionDetail, error) {
	var response TransactionsResponse
	if err := service(*ts).do("GET", "budgets/"+budgetId+"/accounts/"+accountId+"/transactions", nil, nil, &response); err != nil {
		return nil, err
	}
	return response.Data.Transactions, nil
}
...
*/
func (ts *TransactionsService) GetByCategory(budgetId string, categoryId string, sinceDate string) ([]HybridTransaction, error) {
	var response HybridTransactionsResponse
	url := "budgets/" + budgetId + "/categories/" + categoryId + "/transactions"
	queries := make(map[string]string)
	if sinceDate != "" {
		queries["since_date"] = sinceDate
	}

	if err := service(*ts).do("GET", url, queries, nil, &response); err != nil {
		return nil, err
	}
...
func (ts *TransactionsService) GetByPayee(budgetId string, payeeId string) ([]HybridTransaction, error) {
	var response HybridTransactionsResponse
	if err := service(*ts).do("GET", "budgets/"+budgetId+"/payees/"+payeeId+"/transactions", nil, nil, &response); err != nil {
		return nil, err
	}
	return response.Data.Transactions, nil
}
