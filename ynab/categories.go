...
func (cs *CategoriesService) List(budgetId string) ([]CategoryGroupWithCategories, error) {
	...
	if err := service(*cs).do("GET", "budgets/"+budgetId+"/categories", nil, nil, &response); err != nil {
		return nil, err
	}
...
func (cs *CategoriesService) Get(budgetId string, categoryId string) (Category, error) {
	...
	if err := service(*cs).do("GET", "budgets/"+budgetId+"/categories/"+categoryId, nil, nil, &response); err != nil {
		return Category{}, err
	}
